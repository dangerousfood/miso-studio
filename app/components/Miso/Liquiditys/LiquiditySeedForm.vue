  ...
  <card>
    ...
    <div class="mt-5">
      <validation-observer v-if="activeStep === 0" v-slot="{ handleSubmit }">
        <form class="needs-validation" @submit.prevent="handleSubmit(createLiquidityLauncher)">
          <div class="form-row justify-content-md-center">
            <div class="col-lg-6 mr-1">

              <base-input label="TemplateId" name="TemplateId" placeholder="TemplateId"
                          type="text" :rules="`required`"
                          v-model="liquidityDetailsForm.templateId"></base-input>
            </div>
          </div>
          <hr />
          ...
          <div class="col-md-6">
            <card>
              <div slot="header" class="row">
                <div class="col">
                  <h6 class="text-uppercase text-muted ls-1 mb-1">Liquidity Launcher</h6>
                </div>
              </div>
...
export default {
  ...
  methods: {
    async createLiquidityLauncher() {
      ...
      const args = [
        this.liquidityDetailsForm.templateId
      ];
      console.log(this.liquidityDetailsForm.templateId);
      const txHash = await liquidityLauncherCreate('createLiquidityLauncher', args, { from: this.coinbase });
      console.log(txHash);
    },
    subscribeToLiquidityLauncherCreatedEvent() {
      this.liquidityLauncherCreatedEventSubscription = subscribeToLiquidityLauncherCreatedEvent()
          .on("data", async event => {
