      ...
      <div class="px-3 py-3">
        <h6 class="text-sm text-muted m-0">
          <span v-if="transactions.length > 0">
            You have
            <strong class="text-primary">{{numberOfPendingTxs}}</strong> pending
            ...
            <div class="row align-items-center">
              <div class="col-auto">
                ...
                <i
                  class="fas"
                  :class="[
                      {'fa-check-circle text-success': transaction.status == 'CONFIRMED'},
              ...
              <div class="col ml--2">
                <div class="d-flex justify-content-between align-items-center">
                  <div>
                    <h4 class="mb-0 text-sm">{{transaction.transactionHash | shortenAddress(10)}}</h4>
                  </div>
...
export default {
  ...
  computed: {
    ...
    numberOfPendingTxs() {
      return this.transactions.filter(tx => tx.status.indexOf("PENDING") !== -1)
        .length;
    }
  },
  mounted() {
