	...
	>
			...
			<client-only>
				<div
					...
					:class="{ 'py-4': !notDesktopSize }"
					 v-if="!coinbase"
				>
					<span class="text-white">Wallet not detected</span>
				</div>
				<div class="d-flex justify-content-center pb-5 mb-0 pb-lg-0">
					<button
						v-if="!coinbase"
		...
		<div v-if="coinbase">
			...
			<network-modal :show.sync="showNetworkModal">
				<div class="d-flex flex-col h-full w-100 bg-dark-900 rounded overflow-y-auto p-6">
					<div class="relative mb-2">
						<h2 class="text-h5 mt-2.5 font-bold mb-0">Select a Network</h2>
						<div @click="closeModal" class="p-1 absolute top-0 right-0 w-5 h-5 rounded-full border-2 text-primary hover:text-high-emphesis border-primary  flex items-center justify-center cursor-pointer">
					...
					<div class="text-lg text-primary mb-6">
						You are currently browsing <span class="font-bold text-pink">MISO</span>
						<br /> on the <span class="font-bold text-blue">Görli</span> network
					</div>
					<div class="d-flex flex-col space-y-5 overflow-y-auto">
						<button class="flex w-100 rounded p-75 d-flex items-center bg-dark-800 network_btn" v-for="(item, index) in ChainIDs" :key="index">
							<img
								:src="`networks/${item.icon}`"
								alt="Switch Network"
								class="rounded-md mr-2 w-8 h-8"
							/>
							<div class="text-primary font-bold">{{ item.chainName }}</div>
						</button>
					</div>
				</div>
			</network-modal>
...
<style lang="scss">
	...
	.from-blue {
		...
		--tw-gradient-stops: var(--tw-gradient-from), var(--tw-gradient-to, rgba(39, 176, 230, 0)) !important;
	}
	.bg-gradient-to-r {
